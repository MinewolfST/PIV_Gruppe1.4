...
std::vector<std::vector<int>> create_window(std::vector<std::vector<int>> field, int window_size, int x, int y) {  //extract window of given size as given coordinates
if(extra_output){std::cout << "Extracting window of size " << std::to_string(window_size) << " at coordinates " << std::to_string(x) << "," << std::to_string(y) << ":" << std::endl;}
    std::vector<std::vector<int>> window;
        for(int i=0;i<window_size;i++) {
            std::vector<int> row;                //iterate over columns and rows of window
...
int correlate(std::vector<std::vector<int>> window, std::vector<std::vector<int>> window_prime) {  //calculate the correlation value for two given windows
...
std::vector<std::vector<std::vector<int>>> cross_correlate(std::vector<std::vector<int>> field, std::vector<std::vector<int>> field_prime, int window_size, int max_shift) {
    std::vector<std::vector<std::vector<int>>>  vec_field;
    ...
    for(int a=0;a<vec_field_height;a++) {                                           //iterating over vector positions
        std::vector<std::vector<int>> vec_row;
        for(int b=0;b<vec_field_width;b++)  {
            ...
            std::vector<std::vector<int>> window = create_window(field,window_size,x,y);  //extracting window from field
            std::vector<int*> R;
            for(int s_x=-max_shift;s_x<max_shift;s_x++) {                //iterating over shifts until maximum allowed is reached comparing to field prime
                for(int s_y=-max_shift; s_y<max_shift; s_y++) {
                    std::vector<std::vector<int>> window_prime = create_window(field_prime,window_size,x+s_x,y+s_y);
...
int main() {          //test code
    std::vector<std::vector<int>> field = read_file("Date_Messdaten_TVL_96.dat");
    std::vector<std::vector<int>> field_prime = read_file("Date_Messdaten_TVL_97.dat");
    std::vector<std::vector<std::vector<int>>>  vec_field = cross_correlate(field,field_prime,50,10);
    std::cout << std::endl << "Calculations complete, printing vector field: " << std::endl;

    std::vector<std::vector<int>> vel_field;

    for(int i=0;i<vec_field.size();i++) {
        std::vector<int> vel_row;
